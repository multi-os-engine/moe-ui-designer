...
import static org.moe.designer.configurations.Configuration.PREFERENCES_MIN_API;
...
public class LayoutPullParserFactory {
  ...
  public static ILayoutPullParser create(@NotNull final RenderService renderService) {
    ...
    IXmlFile file = renderService.getPsiFile();

    // IntelliJ bug: Claims that
    // folderType can be null below. Suppressed.
...
//        return new MenuPreviewRenderer(renderService, file).render();
  ...
  private static ILegacyPullParser createDrawableParser(IXmlFile file) {
    ...
    String background = IOSPsiUtils.getRootTagAttributeSafely(file, ATTR_BACKGROUND, TOOLS_URI);
    ...
    String scaleType = IOSPsiUtils.getRootTagAttributeSafely(file, ATTR_SCALE_TYPE, TOOLS_URI);

...
import org.moe.designer.configurations.ConfigurationManager;
...
import static org.moe.designer.rendering.PsiProjectListener.isRelevantFile;
import static org.moe.designer.rendering.PsiProjectListener.isRelevantFileType;
...
public final class ResourceFolderRepository extends LocalResourceRepository {
  ...
  private void scanFileResourceFolder(@NotNull PsiDirectory directory, ResourceFolderType folderType, String qualifiers,
    ...
    InitialViewComptrollerService viewControllerService = InitialViewComptrollerService.getInstance(project);
    for (PsiFile file : directory.getFiles()) {
      ...
      //todo MOE load/construct segue tree
      if(folderType.equals(ResourceFolderType.LAYOUT) && file instanceof IXmlFile){
        controller.addDestination(project, file);
        viewControllerService.add(file.getVirtualFile());
      }
    }

    //todo MOE load/construct segue tree
    String projectPath = FileUtil.toSystemDependentName(project.getBasePath());
    String resourcePath = FileUtil.toSystemDependentName(directory.getVirtualFile().getPath());
    if(folderType.equals(ResourceFolderType.LAYOUT) && resourcePath.contains(projectPath)){
...
//       Same handling as rescan, where the initial deletion is a no-op
      ...
      if(folderType != null && folderType.equals(ResourceFolderType.LAYOUT)){
        if(psiFile instanceof IXmlFile){
          Project project = myModule.getProject();
          SegueDestinationController.getInstance(project).addDestination(project, psiFile.getVirtualFile());
        }
      }
    }

...
import static org.moe.designer.configurations.ConfigurationListener.MASK_ALL;
import static org.moe.designer.configurations.ConfigurationListener.MASK_RENDERING;
import static org.moe.designer.rendering.RenderErrorPanel.SIZE_ERROR_PANEL_DYNAMICALLY;
import static com.intellij.designer.designSurface.ZoomType.FIT_INTO;

/**
 * @author Alexander Lobas
 */
public final class IOSDesignerEditorPanel extends DesignerEditorPanel  implements RenderContext,ResourceFolderManager.ResourceFolderListener,OverlayContainer {
    ...
    private final IXmlFile myXmlFile;
    ...
    private final IOSDesignerEditorPanel.LayoutConfigurationListener myConfigListener;
    ...
    private static ViewsMetaManager myViewsMetamanager;
    ...
    public IOSDesignerEditorPanel(@NotNull DesignerEditor editor,
                                      @NotNull Project project,
                                      @NotNull Module module,
                                      @NotNull VirtualFile file) {
        super(editor, project, module, file);

        if (myViewsMetamanager == null)
        {
            myViewsMetamanager = new ViewsMetaManager(project);
        }
//        myTreeDecorator = null;
        ...
        if (facet==null){
            facet = new AndroidFacet(getModule(),"ios-facet",new AndroidFacetConfiguration());
        ...
        mySessionQueue = new MergingUpdateQueue("ios.designer", 10, true, null, editor, null, Alarm.ThreadToUse.OWN_THREAD);
        myXmlFile = (IXmlFile) IOSPsiUtils.getPsiFileSafely(getProject(), myFile);
...
//        AndroidPlatform myPlatform = AndroidPlatform.getPlatform(getModule());
//        try {
...
//        }
    ...
    private void addConfigurationActions() {
        ...
        ConfigurationToolBar.createActions(this);
        designerActionGroup.add(group);
    }

    @Override
    protected DesignerActionPanel createActionPanel() {
        return new IOSDesignerActionPanel(this, myGlassLayer);
    }
...
//        group.add(new AndroidRefactoringActionWrapper("I_nline Layout...", action));
    ...
    private void parseFile(final Runnable runnable) {
        ...
        createRenderer(new ThrowableConsumer<RenderResult, Throwable>() {
            ...
            public void consume(RenderResult result) throws Throwable {
                ...
                if (myRootView == null) {
                    myRootView = new RootView(IOSDesignerEditorPanel.this, 0, 0, result);
                ...
                try {
                    myRootComponent = RadModelBuilder.update(IOSDesignerEditorPanel.this, result, (RadViewComponent) myRootComponent, myRootView);
                } catch (Throwable e) {
    ...
    private void createRenderer(final ThrowableConsumer<RenderResult, Throwable> runnable) {
        ...
        try{
            ...
            mySessionQueue.queue(new Update("render") {
                ...
                public void run() {
                    try {
                        ...
                        RenderContext renderContext = IOSDesignerEditorPanel.this;
                        if (myRendererLock.tryLock()) {
                            try {
                                ...
                                if (service != null) {
                                    ...
                                    logger.error(null, "Blank result created" +
                                            "configuration.android = " + myConfiguration.getTarget().toString(), null);
                                }
                        ...
                        ApplicationManager.getApplication().invokeLater(new Runnable() {
                            ...
                            public void run() {
                                myPsiChangeListener.clear();
                                showError("Internal error occurred in UIPrototyper. Please restart Android Studio", e);
                            }
                        });
                    }
                }
            ...
            showError("Internal error occurred in UIPrototyper. Please restart Android Studio", th);
        }
    ...
    protected Module findModule(Project project, VirtualFile file) {
        ...
        if (module == null) {
            module = IOSPsiUtils.getModuleSafely(myXmlFile);
        }
        return module;
    }
...
//        return ViewsMetaManager.getInstance(getProject()).getPaletteGroups();
        if (myViewsMetamanager == null)
        {
            myViewsMetamanager = new ViewsMetaManager(getProject());
        }
        return myViewsMetamanager.getPaletteGroups();
    }
    ...
    public String getVersionLabel(@Nullable String version) {
        ...
        String name = null;//SdkVersionInfo.getAndroidName(since);
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    //todo update segue tree
                    if(updateProperties){
                        SegueDestinationController controller = SegueDestinationController.getInstance(getProject());
                        controller.reInitDestination();
                    }
    ...
    private Dimension getDesignerViewSize() {
        ...
        if (rootView != null) {
            if (rootView.getShowDropShadow()) {
                size.width -= IOSShadowPainter.SHADOW_SIZE;
                size.height -= IOSShadowPainter.SHADOW_SIZE;
            }
...
//
    ...
    public boolean hasAlphaChannel() {
        return true;//!myRootView.getShowDropShadow();
    }

...
//import org.moe.designer.gradle.IdeaAndroidProject;
import org.moe.designer.gradle.IdeaAndroidProject;
...
import org.moe.designer.android.model.impl.JpsAndroidModuleProperties;
...
public class ResourceFolderManager implements ModificationTracker {
  ...
  public static final String CUSTOM_ELEMENTS_DIR = "custom_elements";
  ...
  public List<VirtualFile> getFolders() {
    ...
    if (myResDirCache == null || myResDirCache.size() == 0) {
  ...
  private List<VirtualFile> computeFolders() {
   final boolean isGradleProject = true;//myFacet.isGradleProject();
    System.out.println("Always Gradle!");
    if (isGradleProject) {
        ...
        if (state != null) {
          ...
          if (path != null) {
            ...
            if (dir != null) {
              ...
              mainRes = '/' + FD_SOURCES + '/' + FD_MAIN + '/' + FD_XRT_RESOURCES;
              dir =  AndroidRootUtil.getFileByRelativeModulePath(myFacet.getModule(), mainRes, true);
              if (dir != null) {
                resDirectories.add(dir);
              }
              else{
...
//          });
      ...
      try {
        ...
        customPath = IOSPsiUtils.getPathOfJarResource(CUSTOM_ELEMENTS_DIR);
        res = LocalFileSystem.getInstance().findFileByPath(customPath);
        if (res == null) {
          URL urlPath = ResourceFolderManager.class.getClassLoader().getResource(CUSTOM_ELEMENTS_DIR);
          res = LocalFileSystem.getInstance().findFileByPath(urlPath.getFile());
          if (!res.exists() && !res.isDirectory()){
            throw new Exception( " Custom elements not found in " + customPath);
          }
        }
        else{
          System.out.println("Found custom_elemnts directory " + res.toString());
        }
        res.refresh(false, true);
        resDirectories.add(res);
      } catch (Exception e){
        try {
          throw new Exception(ResourceFolderManager.class.getName() + " not found custom resources in " + customPath, e);
        } catch (Exception e1) {
          e1.printStackTrace();
        }
        e.printStackTrace();

...
//    }
  ...
  private AndroidSdkData(@NotNull LocalSdk localSdk) {
    myLocalSdk = localSdk;
    mySettingsController = new SettingsController(null);//new NullLogger() /* TODO */);
    myRemoteSdk = new RemoteSdk(mySettingsController);
    String path = localSdk.getPath();
    assert path != null;
    myPlatformToolsRevision = 10;//AndroidCommonUtils.parsePackageRevision(path, SdkConstants.FD_PLATFORM_TOOLS);
    mySdkToolsRevision = 10;//AndroidCommonUtils.parsePackageRevision(path, SdkConstants.FD_TOOLS);
    myDeviceManager = DeviceManager.createInstance(localSdk.getLocation(),null);// new MessageBuildingSdkLog());
  }

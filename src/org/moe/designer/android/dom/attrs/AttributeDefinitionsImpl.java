 ...
 */
package org.moe.designer.android.dom.attrs;
...
public class AttributeDefinitionsImpl implements AttributeDefinitions {
  ...
  private void addAttrsFromFile(XmlFile file) {
    ...
    for (XmlTag tag : rootTag.getSubTags()) {
      ...
      if (tagName.equals(TAG_XRT_ATTR)) {
        parseAttrTag(tag, null);
      }
  ...
  private AttributeDefinition parseAttrTag(XmlTag tag, @Nullable String parentStyleable) {
    ...
    if (values.length > 0) {
      ...
      if(values.length == 0){
        values = tag.findSubTags(TAG_HANDLER);
        if (values.length > 0) {
          formats.add(AttributeFormat.Handler);
        }
      }
    }
    ...
    if (def == null) {
      ...
      def = tag.getName().equals("xrt_attr") ? new XRTAttributeDifinition(name):new AttributeDefinition(name);
  ...
  private void parseAndAddValues(AttributeDefinition def, XmlTag[] values) {
    for (XmlTag value : values) {
      ...
      if (valueName == null) {
        ...
        final String intNameValue = value.getAttributeValue(INT_NAME_VALUE);
        if(intNameValue != null){
          def.addInternalName(valueName, intNameValue);
        }
      }
    }
  }
  ...
  private void parseDeclareStyleableTag(XmlTag tag, Map<StyleableDefinitionImpl, String[]> parentMap) {
    ...
    for (XmlTag subTag : tag.findSubTags(TAG_XRT_ATTR)) {
      parseStyleableAttr(def, subTag);
    }
  }

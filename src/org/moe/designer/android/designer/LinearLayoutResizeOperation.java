 ...
 */
package org.moe.designer.android.designer;
...
//    } else {
      //todo new strategy for set weight
    String currentWidth = node.getAttribute(ATTR_LAYOUT_WIDTH, ANDROID_URI);
    String currentHeight = node.getAttribute(ATTR_LAYOUT_HEIGHT, ANDROID_URI);

      if(isVertical(layout) && context.fillHeight){
        node.setAttribute(ATTR_LAYOUT_WEIGHT, ANDROID_URI, "1.0");
      }
      else if(!isVertical(layout) && context.fillWidth){
        node.setAttribute(ATTR_LAYOUT_WEIGHT, ANDROID_URI, "1.0");
      }
      else if (!currentWidth.equals("fill_parent") && !currentWidth.equals("match_parent") &&
               !currentHeight.equals("fill_parent") && !currentHeight.equals("match_parent")){
...
//    }
  ...
  private class LinearResizeContext extends ResizeContext {
    ...
    private Map<XmlTag, Dimension> computeUnweightedSizes() {
      ...
      if (parentTag != null) {
        RenderService service = IOSDesignerUtils.getRenderService(myContext.getArea());
        if (service == null) {
          return null;
        }

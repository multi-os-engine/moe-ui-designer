...
import org.moe.designer.propertyTable.editors.*;
import org.moe.designer.propertyTable.renderers.*;
...
public class AttributeProperty extends PropertyWithNamespace implements IXmlAttributeLocator {
  ...
  protected final PropertyRenderer myRenderer;
  protected final PropertyEditor myEditor;
  protected String myTooltip;
  protected String myJavadocText;
  ...
  public AttributeProperty(@Nullable Property parent, @NotNull String name, @NotNull AttributeDefinition definition) {
    ...
    if("viewController".equals(getName())){
      myRenderer = new ViewControllerRenderer(formats);
      myEditor = new ViewControllerEditor(formats);
      return;
    }
    if("iboutlet".equals(getName())){
      myRenderer = new IBOutletRenderer(formats);
      myEditor = new IBOutletEditor(formats);
      return;
    }
    if (formats.size() == 1) {
...
//        myEditor = createResourceEditor(definition, formats);
        myRenderer = new EventHandlerRenderer(formats);
        myEditor = new XRTEventHandlerEditor(formats, true);
        return;
      }
    }
    myRenderer = createResourceRenderer(definition, formats);

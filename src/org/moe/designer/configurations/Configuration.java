...
import org.moe.designer.rendering.api.Features;
...
//  @Nullable
  ...
  protected Configuration(@NotNull ConfigurationManager manager, @Nullable VirtualFile file, @NotNull FolderConfiguration editedConfig) {
    ...
    if (isOrientationSpecificLayout()) {
     ...
     String value = IOSPsiUtils.getAndroidSDKPath(manager.getProject());
    if (value != null) {
      myTarget = IOSPsiUtils.getAndroidTarget(manager.getProject());
      if (file != null) {
        if (ResourceHelper.getFolderType(file) == ResourceFolderType.XML) {
          myPsiFile = IOSPsiUtils.getPsiFileSafely(manager.getProject(), file);
...
//  }
  ...
  private Device computeBestDevice() {
    Device[] recentDevices = null;//myManager.getRecentDevices()

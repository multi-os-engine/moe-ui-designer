...
//import org.moe.designer.rendering.ViewInfo;
import org.moe.designer.IOSDesignerUtils;
import org.moe.designer.android.designer.AndroidMetaModel;
import org.moe.designer.designSurface.TransformedComponent;
import org.moe.designer.propertyTable.renderers.XRTAttributeProperty;
import org.moe.designer.rendering.IncludeReference;
...
import org.moe.designer.Insets;
import javax.swing.*;
import java.awt.*;
import java.util.*;
import java.util.List;

import static org.moe.designer.rendering.IncludeReference.ATTR_RENDER_IN;
...
public class RadViewComponent extends RadVisualComponent {
  ...
  private final String INPLACE_PROPERTY_NAME = "text"; //configure property for popup on element double click
  ...
  public AndroidMetaModel getMetaModel() {
    MetaModel model =  super.getMetaModel();
//    return new AndroidMetaModel(model.getModel(),model.getTarget(),model.getTag());
//    return model;
    return (AndroidMetaModel)model;
  }
  //////////////////////////////////////////////////////////////////////////////////////////
  //
  //
  ...
  public Dimension calculateWrapSize(EditableArea area) {
    ...
    if (parentTag != null) {
      RenderService service = IOSDesignerUtils.getRenderService(area);
      if (service == null) {
        return null;
      }
  ...
  public boolean canDelete() {
    return isTableViewHeader() && !isBackground() && super.canDelete();
  }
  ...
  public List<Property> getInplaceProperties() throws Exception {
    ...
    Property idProperty = PropertyTable.findProperty(myProperties, INPLACE_PROPERTY_NAME);
    if (idProperty != null) {
...
//  }
  ...
  /**
   * Like {@link #toModel(Component, Rectangle)}, but
   ...
   * pixels (with {@link #toModel(Component, Rectangle)},
  ...
  /**
   * Like {@link #toModel(Component, Point)}, but
   * also translates from pixels in the source, to device independent pixels
   * in the model.
   * <p>
   * A lot of client code needs to compute model dp by doing arithmetic on
   * bounds in the tool (which are scaled). By first computing to model
   * pixels (with {@link #toModel(Component, Point)},
   * and <b>then</b> computing the dp from there, you introduce two levels
   * of rounding.
   * <p>
   * This method performs both computations in a single go which reduces
  ...
  /**
   * Like {@link #toModel(Component, Dimension)}, but
   * also translates from pixels in the source, to device independent pixels
   * in the model.
   * <p>
   * A lot of client code needs to compute model dp by doing arithmetic on
   * bounds in the tool (which are scaled). By first computing to model
   * pixels (with {@link #toModel(Component, Dimension)},
   * and <b>then</b> computing the dp from there, you introduce two levels
   * of rounding.
   * <p>
   * This method performs both computations in a single go which reduces
  ...
  /**
   * Like {@link #getBounds(Component)}, but rather than applying to the
   ...
   */
  public boolean addPopupActions(@NotNull IOSDesignerEditorPanel designer,
  ...
  public void sortWithPriorityExceptions(final List<String> priorityExceptions){
    if(myProperties != null){
      Collections.sort(myProperties, new Comparator<Property>() {
        @Override
        public int compare(Property p1, Property p2) {
          if((priorityExceptions.contains(p1.getName())&& priorityExceptions.contains(p2.getName())) ||
             (p1.getClass() == XRTAttributeProperty.class && p2.getClass() == XRTAttributeProperty.class)){
            return p1.getName().compareTo(p2.getName());
          }
          else if(priorityExceptions.contains(p1.getName()) || p1.getClass() == XRTAttributeProperty.class){
            return -1;
          }
          else if(priorityExceptions.contains(p2.getName()) || p2.getClass() == XRTAttributeProperty.class){
            return 1;
          }
          else{
            return p1.getName().compareTo(p2.getName());
  ...
  public boolean isTableViewHeader(){
    if(myTag.getName().equals("com.android.sdklib.widgets.iOSTextView")){
      RadComponent parent = getParent();
      if(parent instanceof RadViewComponent){
        if(((RadViewComponent)parent).getTag().getName().equals("com.android.sdklib.widgets.iOSTableView")){

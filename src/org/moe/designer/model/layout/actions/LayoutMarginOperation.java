 ...
 */
package org.moe.designer.model.layout.actions;
...
import org.moe.designer.designSurface.graphics.*;
...
public class LayoutMarginOperation implements EditOperation {
  ...
  protected void fillTextFeedback() {
    ...
    if (direction == Position.WEST) { // left
      myTextFeedback.append(IOSDesignerUtils.pxToDpWithUnits(area, myMargins.left - moveDelta.width));
    }
    else if (direction == Position.EAST) { // right
      myTextFeedback.append(IOSDesignerUtils.pxToDpWithUnits(area, myMargins.right + sizeDelta.width));
    }
    else if (direction == Position.NORTH) { // top
      myTextFeedback.append(IOSDesignerUtils.pxToDpWithUnits(area, myMargins.top - moveDelta.height));
    }
    else if (direction == Position.SOUTH) { // bottom
      myTextFeedback.append(IOSDesignerUtils.pxToDpWithUnits(area, myMargins.bottom + sizeDelta.height));
    }
  }
  ...
  private void setValue(XmlTag tag, String name, int pxValue) {
    int value = IOSDesignerUtils.pxToDp(myContext.getArea(), pxValue);
    if (value == 0) {

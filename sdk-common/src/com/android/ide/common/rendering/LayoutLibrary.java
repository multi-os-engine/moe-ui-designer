...
import com.android.ide.common.rendering.legacy.CustomURLClassLoader;
    ...
    public static LayoutLibrary load(String layoutLibJarOsPath, ILogger log, String toolName) {
        ...
        try {
            ...
            if (f.isFile() == false) {
                ...
                //TODO MOE
                List<URL> customUrls = new ArrayList<URL>(Arrays.asList(urls));
                if(LayoutLibrary.class.getClassLoader() instanceof UrlClassLoader){
                    UrlClassLoader urlloader = (UrlClassLoader) LayoutLibrary.class.getClassLoader();
                    customUrls.addAll(urlloader.getUrls());

                    Iterator<URL> iterator = customUrls.iterator();
                    while(iterator.hasNext()){
                        URL next = iterator.next();
                        if(next.getPath().contains("android.jar")){
                            iterator.remove();
                        }
                    }
                }
...
//                classLoader = new CustomElementClassLoader(customUrls.toArray(new URL[]{}), new URLClassLoader(customUrls.toArray(new URL[]{}), LayoutLibrary.class.getClassLoader()));
                classLoader = new CustomElementClassLoader(customUrls.toArray(new URL[]{}), new CustomURLClassLoader(customUrls.toArray(new URL[]{}), LayoutLibrary.class.getClassLoader()));
